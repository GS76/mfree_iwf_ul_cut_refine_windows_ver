...
static constexpr unsigned int max_SON2D = 6;

class body;

class adaptivity {
  public:
	enum refine_criteria { velocity, div_velo, vonmises, plastic_strain, temperature, position, moving_frame, num_nbh };

	enum pattern { triangular, cubic_basic, cubic, hexagonal };
	...
	adaptivity(double alpha_dx, double beta_h, double v, double div_v, double SvM, double epsPl, double T, glm::dvec2 xy_min,
  ...
  private:
	void calculate_son_positions_triangular(double x_DAD, double y_DAD, double dx, std::array<double, max_SON2D> &x_SON,
											std::array<double, max_SON2D> &y_SON) const;
	void assign_son_properties(const std::array<double, max_SON2D> &x_SON, const std::array<double, max_SON2D> &y_SON, double h_DAD,
	...
	particle create_son_particle(unsigned int son_idx, double x, double y, double m, double h, unsigned int step,
