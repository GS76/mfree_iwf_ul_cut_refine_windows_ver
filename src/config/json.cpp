...
struct parser {
	...
	json_value parse_value() {
		...
		if (c == '-' || std::isdigit((unsigned char) c)) return json_value(parse_number());
	...
	json_value parse_object() {
		...
		if (consume('}')) return json_value(std::move(obj));
		while (true) {
			skip_ws();
	...
	json_value parse_array() {
		...
		if (consume(']')) return json_value(std::move(arr));
		int idx = 0;
		while (true) {
	...
	std::string parse_string() {
		...
		while (true) {
			char c = get();
			if (c == '\0') fail("Unexpected end of input in string");
			if (c == '"') break;
			if (c == '\\') {
				char e = get();
				if (e == '\0') fail("Unexpected end of input in string escape");
				switch (e) {
	...
	double parse_number() {
		...
		size_t start = i;
		if (peek() == '-') get();
		if (!std::isdigit((unsigned char) peek())) fail("Invalid number");
		if (peek() == '0') {
			get();
		} else {
	...
	json_value parse_true() {
		expect('t');
		if (get() != 'r' || get() != 'u' || get() != 'e') fail("Invalid literal");
		return json_value(true);
	}

	json_value parse_false() {
		expect('f');
		if (get() != 'a' || get() != 'l' || get() != 's' || get() != 'e') fail("Invalid literal");
		return json_value(false);
	}

	json_value parse_null() {
		expect('n');
		if (get() != 'u' || get() != 'l' || get() != 'l') fail("Invalid literal");
