	...
	for (int ii = 0; ii < static_cast<int>(n); ii++) {
		...
		for (unsigned int j = 0; j < particles[i].num_nbh; j++) {
			...
			if (rhoi < 0.) {
				...
				printf("  stress     : Sxx=%e  Sxy=%e  Syy=%e  p=%e\n", particles[i].Sxx, particles[i].Sxy, particles[i].Syy,
					   particles[i].p);
				printf("  refine_step=%u  last_refine_at=%u  steps_since_split=%u  num_nbh=%u\n", particles[i].refine_step,
					   particles[i].last_refine_at, step - particles[i].last_refine_at, particles[i].num_nbh);
				fflush(stdout);
				exit(-1);
			}
			...
			if (rhoj < 0.) {
				...
				printf("  stress     : Sxx=%e  Sxy=%e  Syy=%e  p=%e\n", particles[jdx].Sxx, particles[jdx].Sxy, particles[jdx].Syy,
					   particles[jdx].p);
				printf("  refine_step=%u  last_refine_at=%u  steps_since_split=%u  num_nbh=%u\n", particles[jdx].refine_step,
					   particles[jdx].last_refine_at, step - particles[jdx].last_refine_at, particles[jdx].num_nbh);
				printf("  seen by i  : idx=%u  x=%e  y=%e  h=%e  rho=%e  refine_step=%u\n", particles[i].idx, particles[i].x,
					   particles[i].y, particles[i].h, particles[i].rho, particles[i].refine_step);
				fflush(stdout);
				exit(-1);
			}
