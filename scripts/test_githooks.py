...
def main() -> int:
    ...
    try:
        ...
        for p in [
            install_ps1,
            install_sh,
            repo_root / "scripts" / "check_editorconfig_basic.py",
            repo_root / "scripts" / "check_clang_format.py",
        ]:
        ...
        if os.name == "nt":
            run(
                [
                    ...
                    "Bypass",
                    "-File",
                    str(scripts_dir / "install_githooks.ps1"),
                ],
                cwd=tmp,
            )
        else:
        ...
        rc, out = run(
            ["git", "commit", "-m", "test: should block"], cwd=tmp, check=False
        )
        if rc == 0:
            raise RuntimeError(
                "expected commit to be blocked when no staged changes exist"
            )
        ...
        if rc == 0:
            raise RuntimeError(
                "expected commit to be blocked by commit-msg rule (missing ':')"
            )

        rc, out = run(
            ["git", "commit", "-m", "hook: allow staged change"], cwd=tmp, check=False
        )
        if rc != 0:
            raise RuntimeError(
                f"expected commit to succeed with valid message and staged changes\n{out}"
            )
