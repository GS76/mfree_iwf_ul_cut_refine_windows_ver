...
function Select-FrameIndices([string]$ModelDir, [int]$Count) {
	...
	$idx = @()
	foreach ($f in $files) {
		if ($f.BaseName -match '^out_(\d+)$') { $idx += [int]$Matches[1] }
	}
	$idx = $idx | Sort-Object -Unique
	if ($idx.Count -eq 0) { return @() }
	if ($Count -le 0) { return $idx }
	if ($idx.Count -le $Count) { return $idx }
	$out = @()
	for ($i = 0; $i -lt $Count; $i++) {
		$t = if ($Count -eq 1) { 0.0 } else { $i / ($Count - 1.0) }
...
foreach ($m in 1..4) {
	$modelDir = Join-Path $ResultsRoot "model_$m"
	...
	$bcReport = $null
	if (Test-Path $bcReportPath) { $bcReport = Get-Content -Raw $bcReportPath | ConvertFrom-Json }
	$valSummary = $null
	if (Test-Path $valSummaryPath) { $valSummary = Get-Content -Raw $valSummaryPath | ConvertFrom-Json }
	$lastMetrics = $null
	if (Test-Path $metricsPath) { $lastMetrics = (Import-Csv $metricsPath | Select-Object -Last 1) }
	$lastThermal = $null
	if (Test-Path $thermalPath) { $lastThermal = (Import-Csv $thermalPath | Select-Object -Last 1) }

	$modelSummaries += [pscustomobject]@{
		model = $m
		results_dir = $modelDir
		bc_top_nodes = if ($bcReport) { $bcReport.top_nodes } else { $null }
		bc_rear_nodes = if ($bcReport) { $bcReport.rear_nodes } else { $null }
		bc_temp_err_top_K = if ($bcReport) { $bcReport.max_abs_temp_err_K.top } else { $null }
		bc_temp_err_rear_K = if ($bcReport) { $bcReport.max_abs_temp_err_K.rear } else { $null }
		wp_umax_m = if ($lastMetrics) { [double]$lastMetrics.wp_umax } else { $null }
		wp_svm_max = if ($lastMetrics) { [double]$lastMetrics.wp_svm_max } else { $null }
		wp_epspl_max = if ($lastMetrics) { [double]$lastMetrics.wp_epspl_max } else { $null }
		wp_contact_pmax = if ($lastMetrics) { [double]$lastMetrics.wp_contact_pmax } else { $null }
		P_cond_W = if ($lastThermal) { [double]$lastThermal.P_cond_W } else { $null }
		P_fric_W = if ($lastThermal) { [double]$lastThermal.P_fric_W } else { $null }
		tool_vel_x = if ($lastThermal) { [double]$lastThermal.tool_vel_x } else { $null }
		max_disp_validation = if ($valSummary -and $valSummary.workpiece) { $valSummary.workpiece.max_displacement } else { $null }
	}
}
...
if ($GenerateReport) {
	...
	$lines = @()
	$lines += "# FE Advancement Procedure Report (Models 1-4)"
	$lines += ""
	$lines += "- Generated: $now"
	...
	foreach ($r in $modelSummaries) {
		...
		$lines += ""
		$lines += "![](./model_$($r.model)/pv_bc_png/edge_nodes.png)"
		$lines += ""
		$lines += "![](./model_$($r.model)/pv_bc_png/velocity_glyphs.png)"
		$lines += ""
		...
		foreach ($p in $pngs) {
			$lines += "![](./model_$($r.model)/pv_adv_png/$($p.Name))"
			$lines += ""
		}
	}
	$pdfStatus = "skipped"
	...
	$note += "- PDF: $pdfStatus"
	if ($pdfError) { $note += "- PDF note: $pdfError" }
